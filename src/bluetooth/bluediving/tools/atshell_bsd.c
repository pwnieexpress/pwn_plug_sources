 ...
 *
 *
 *  $Id: atshell_bsd.c,v 1.1.1.1 2005/12/27 14:31:21 bytebeater Exp $
 */
...
{
	struct sockaddr_rfcomm remote_addr, local_addr;
	int s;

	if ((s = socket(PF_BLUETOOTH, SOCK_STREAM, BLUETOOTH_PROTO_RFCOMM)) < 0) {
		printf("Can't create socket. %s (%d)\n", strerror(errno), errno);
		return -1;
	}

	memset(&local_addr, 0, sizeof(local_addr));
	local_addr.rfcomm_family = AF_BLUETOOTH;
	bacpy(&local_addr.rfcomm_bdaddr, NG_HCI_BDADDR_ANY);
	...
	remote_addr.rfcomm_family = AF_BLUETOOTH;
	bacpy(&remote_addr.rfcomm_bdaddr, bdaddr);
	remote_addr.rfcomm_channel = channel;
