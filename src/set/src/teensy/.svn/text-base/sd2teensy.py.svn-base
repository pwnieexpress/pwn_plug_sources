...
if not os.path.isfile(path): 
        while 1:
                PrintWarning("Filename not found, try again")
                path = raw_input(setprompt(["6"], "Path to the file you want deployed on the teensy SDCard"))
                if os.path.isfile(path): break

PrintWarning("Note: This will only deliver the payload, you are in charge of creating the listener if applicable.")
PrintStatus("Converting the executable to a hexadecimal form to be converted later...")
