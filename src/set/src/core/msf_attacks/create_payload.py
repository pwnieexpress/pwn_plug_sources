...
show_payload_menu1 = CreateMenu(create_payloads_text, create_payloads_menu)
...
if exploit == "exploit/windows/fileformat/adobe_pdf_embedded_exe" or exploit == "exploit/windows/fileformat/adobe_pdf_embedded_exe_nojs":
    PrintInfo("Default payload creation selected. SET will generate a normal PDF with embedded EXE.")
    print """
...
if exploit_counter == 0:
    ...
    show_payload_menu3 = CreateMenu(payload_menu_3_text, payload_menu_3)
    ...
    if exploit != 'custom/exe/to/vba/payload':
        ...
        PrintStatus("Payload creation complete.")
        time.sleep(1)
        PrintStatus("All payloads get sent to the %s directory" % (outfile))
    if exploit == 'custom/exe/to/vba/payload':
