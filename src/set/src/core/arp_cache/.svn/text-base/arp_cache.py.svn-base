...
for line in config:
        ...
        if match1: 
                setcore.PrintInfo("ARP Cache Poisoning is set to " + core.bcolors.GREEN + "ON" + core.bcolors.ENDC)
                ettercapchoice='y'

        # check for dsniff choice here
        match2=re.search("DSNIFF=ON", line)
        if match2:
                setcore.PrintInfo("DSNIFF DNS Poisoning is set to " + setcore.bcolors.GREEN + "ON" + setcore.bcolors.ENDC)
                dsniffchoice = 'y'
                ettercapchoice = 'n'
...
"""
                setcore.PrintInfo("Example: http://www.google.com")
...
"""
                ...
                filewrite.close()
                setcore.PrintError("LAUNCHING DNSSPOOF DNS_SPOOF ATTACK!")
                # spawn a child process
                os.chdir(cwd)
                # time.sleep(5)
                ...
                log(error)
                # print error message
                setcore.PrintError("ERROR:An error has occurred:")
