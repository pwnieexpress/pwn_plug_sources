...
def launch(cidrBlock,lowport,highport):
        PrintStatus("SET is now scanning the IPs specified... please be patient.")    
        if not validateCIDRBlock(cidrBlock):
                # validate its really an ip address if solo
                ip_check = is_valid_ip(cidrBlock)
                if ip_check != False:
                        PrintStatus("CIDR notation not specified. Trying single IP address.")
                        scan(cidrBlock, int(lowport), int(highport))
                        return host_list
                else:
                        PrintWarning("Invalid IP Address, try again.")
        else:
